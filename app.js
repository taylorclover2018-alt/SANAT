...
/*  
===========================================================
SUPER APP.JS — PARTE 38.1
PRESENÇAS NOS EVENTOS + EVENTOS POR ROTA
===========================================================
*/


// ---------------------------------------------------------
// GARANTIR CAMPOS NOVOS EM EVENTOS EXISTENTES
// ---------------------------------------------------------
function garantirEventosComRotasEPresencas() {

    if (!state.calendario) return;

    state.calendario.eventos.forEach(e => {

        // lista de rotas vinculadas
        if (!e.rotas) e.rotas = [];

        // lista de presenças
        if (!e.presencas) e.presencas = []; 
        // formato:
        // { usuario: "taylor", status: "vou" }
    });
...
// MARCAR PRESENÇA
// ---------------------------------------------------------
function marcarPresenca(eventoId, status) {
    ...
    let p = e.presencas.find(x => x.usuario === u);
    ...
    notificar("Presença atualizada!", "success");
    auditar("Atualizou presença", `${e.titulo} → ${status}`, "acao");

    mudarPagina("editarEvento", eventoId);
}


// ---------------------------------------------------------
// RENDERIZAR PRESENÇAS
// ---------------------------------------------------------
function renderPresencasEvento(e) {

    const vou = e.presencas.filter(x => x.status === "vou").map(x => x.usuario);
    const naoVou = e.presencas.filter(x => x.status === "nao_vou").map(x => x.usuario);
    const talvez = e.presencas.filter(x => x.status === "talvez").map(x => x.usuario);

    return `
        <div class="card">
            <h3>Presenças</h3>

            <p><b>Vou (${vou.length}):</b> ${vou.join(", ") || "—"}</p>
            <p><b>Não vou (${naoVou.length}):</b> ${naoVou.join(", ") || "—"}</p>
            <p><b>Talvez (${talvez.length}):</b> ${talvez.join(", ") || "—"}</p>

            <hr>

            <h4>Minha resposta:</h4>

            <button class="btn btn-primary" onclick="marcarPresenca(${e.id}, 'vou')">Vou</button>
            <button class="btn btn-secondary" onclick="marcarPresenca(${e.id}, 'talvez')">Talvez</button>
            <button class="btn btn-danger" onclick="marcarPresenca(${e.id}, 'nao_vou')">Não vou</button>
        </div>
    `;
}
...
// ---------------------------------------------------------
// RENDERIZAR ROTAS DO EVENTO
// ---------------------------------------------------------
function renderRotasEvento(e) {

    const opcoesRotas = state.rotas
        .map(r => `
            <label style="display:block;">
                <input 
                    type="checkbox" 
                    value="${r.nome}" 
                    ${e.rotas.includes(r.nome) ? "checked" : ""}
                    onchange="toggleRotaEvento(${e.id}, '${r.nome}')"
                >
                ${r.nome}
            </label>
        `)
...
// ---------------------------------------------------------
// ADICIONAR / REMOVER ROTA DO EVENTO
// ---------------------------------------------------------
function toggleRotaEvento(eventoId, rota) {

    const e = state.calendario.eventos.find(x => x.id == eventoId);
    if (!e) return;

    if (e.rotas.includes(rota)) {
        e.rotas = e.rotas.filter(x => x !== rota);
    } else {
        e.rotas.push(rota);
    }
    ...
    notificar("Rotas atualizadas!", "info");
    auditar("Alterou rotas do evento", `${e.titulo} → ${rota}`, "acao");
...
// ---------------------------------------------------------
// EXTENDER A PÁGINA DE EDIÇÃO DE EVENTO
// ---------------------------------------------------------
const paginaEditarEventoOriginal_381 = paginaEditarEvento;
paginaEditarEvento = function (id) {

    const base = paginaEditarEventoOriginal_381(id);
...
// ---------------------------------------------------------
// EXTENDER A PÁGINA DO DIA PARA MOSTRAR PRESENÇAS
// ---------------------------------------------------------
const paginaDiaCalendarioOriginal_381 = paginaDiaCalendario;
paginaDiaCalendario = function (dataStr) {

    let html = paginaDiaCalendarioOriginal_381(dataStr);

    // nada a alterar aqui, pois a edição já mostra presenças
    return html;
};
...
// INTEGRAR AO RENDER
// ---------------------------------------------------------
const renderOriginalParte381 = render;
render = function (param1, param2) {

    garantirEventosComRotasEPresencas();

    renderOriginalParte381(param1, param2);
};
/*  
===========================================================
SUPER APP.JS — PARTE 39/20
SISTEMA DE TAREFAS INTERNAS (TO‑DO CORPORATIVO)
===========================================================
*/
...
function garantirTarefas() {
    ...
    if (!state.tarefas) {
        ...
        //   criador, responsaveis:[], rotas:[], prazo, dataCriacao
        // }
    }
}
...
function paginaTarefas() {
    ...
    const minhas = state.tarefas.filter(t => t.responsaveis.includes(u));
    const criadas = state.tarefas.filter(t => t.criador === u);
    const concluidas = state.tarefas.filter(t => t.status === "concluida");

    return `
        <div class="container">
            ${UI.logo()}
            ${UI.titulo("Tarefas Internas")}

            <button class="btn btn-primary" onclick="mudarPagina('novaTarefa')">Nova Tarefa</button>

            <div class="card">
                <h3>Minhas Tarefas</h3>
                ${renderListaTarefas(minhas)}
            </div>

            <div class="card">
                <h3>Tarefas que criei</h3>
                ${renderListaTarefas(criadas)}
            </div>

            <div class="card">
                <h3>Concluídas</h3>
                ${renderListaTarefas(concluidas)}
            </div>
...
function renderListaTarefas(lista) {
    ...
    if (lista.length === 0) return "<p>Nenhuma tarefa.</p>";

    return lista.map(t => `
        <div class="card" style="border-left:6px solid ${corPrioridade(t.prioridade)};">
            <b>${t.titulo}</b> 
            ${botaoFavorito("tarefas", t.id)}
            <br>
            <small>Status: ${t.status}</small><br>
            <small>Prioridade: ${t.prioridade}</small><br>
            <button class="btn btn-secondary" onclick="mudarPagina('editarTarefa', ${t.id})">Abrir</button>
        </div>
    `).join("");
}
...
function paginaNovaTarefa() {
    ...
    const opcoesUsuarios = state.usuarios
    ...
    return `
        <div class="container">
            ...
            <div class="card">
                ...
                <select id="tResponsaveis" multiple>
                    ${opcoesUsuarios}
                </select>

                <label>Rotas relacionadas:</label>
                <select id="tRotas" multiple>
                    ${opcoesRotas}
                </select>

                <label>Prazo (opcional):</label>
                <input id="tPrazo" type="date">
...
function salvarNovaTarefa() {
    ...
    const prioridade = document.getElementById("tPrioridade").value;

    const responsaveis = Array.from(document.getElementById("tResponsaveis").selectedOptions)
        .map(o => o.value);

    const rotas = Array.from(document.getElementById("tRotas").selectedOptions)
        .map(o => o.value);

    const prazo = document.getElementById("tPrazo").value;

    if (!titulo) {
        notificar("Título é obrigatório.", "error");
        return;
    }
    ...
    notificar("Tarefa criada!", "success");
    auditar("Criou tarefa", titulo, "acao");
...
// ---------------------------------------------------------
function paginaEditarTarefa(id) {
    ...
    return `
        <div class="container">
            ${UI.logo()}
            ${UI.titulo("Tarefa: " + t.titulo)}

            <div class="card">
                <p><b>Descrição:</b> ${t.descricao || "—"}</p>
                <p><b>Prioridade:</b> ${t.prioridade}</p>
                <p><b>Status:</b> ${t.status}</p>
                <p><b>Criador:</b> ${t.criador}</p>
                <p><b>Responsáveis:</b> ${t.responsaveis.join(", ") || "—"}</p>
                <p><b>Rotas:</b> ${t.rotas.join(", ") || "—"}</p>
                <p><b>Prazo:</b> ${t.prazo || "—"}</p>
            </div>

            <div class="card">
                <h3>Alterar Status</h3>
                <button class="btn btn-primary" onclick="alterarStatusTarefa(${id}, 'pendente')">Pendente</button>
                <button class="btn btn-primary" onclick="alterarStatusTarefa(${id}, 'andamento')">Em andamento</button>
                <button class="btn btn-success" onclick="alterarStatusTarefa(${id}, 'concluida')">Concluída</button>
                <button class="btn btn-danger" onclick="alterarStatusTarefa(${id}, 'cancelada')">Cancelada</button>
            </div>
...
function alterarStatusTarefa(id, status) {
    ...
    notificar("Status atualizado!", "success");
    auditar("Alterou status da tarefa", `${t.titulo} → ${status}`, "acao");

    mudarPagina("editarTarefa", id);
}
...
function inserirBotaoTarefas() {
    ...
    if (!menu.innerHTML.includes("Tarefas")) {
        menu.innerHTML = `
            <button class="btn btn-secondary" onclick="mudarPagina('tarefas')">
                📝 Tarefas
            </button>
        ` + menu.innerHTML;
    }
...
// INTEGRAR AO RENDER
// ---------------------------------------------------------
const renderOriginalParte39 = render;
render = function (param1, param2) {
    ...
    if (state.paginaAtual === "editarTarefa") {
        renderSeguro(paginaEditarTarefa(param1));
        inserirNavegacaoInteligente();
        return;
    }

    renderOriginalParte39(param1, param2);

    inserirBotaoTarefas();
};
/*  
===========================================================
SUPER APP.JS — PARTE 40/20
SISTEMA DE ARQUIVOS INTERNOS (DRIVE CORPORATIVO)
===========================================================
*/


// ---------------------------------------------------------
// GARANTIR ESTRUTURA DO DRIVE
// ---------------------------------------------------------
function garantirDrive() {
    ...
    if (!state.drive) {
        state.drive = {
            ...
            // formato:
            // {
            //   id, nome, tipo, tamanho, conteudoBase64,
            //   categoria, criador, dataUpload
            // }
        };
    }
}
...
function paginaDrive() {
    ...
    const categorias = [...new Set(state.drive.arquivos.map(a => a.categoria))];

    return `
        <div class="container">
            ${UI.logo()}
            ${UI.titulo("Drive Interno")}

            <button class="btn btn-primary" onclick="mudarPagina('uploadArquivo')">Enviar Arquivo</button>

            <div class="card">
                <h3>Categorias</h3>
                ${categorias.length === 0 ? "<p>Nenhuma categoria.</p>" : ""}
                ${categorias.map(c => `
                    <button class="btn btn-secondary" onclick="mudarPagina('categoriaDrive','${c}')">
                        📁 ${c}
                    </button>
                `).join("")}
            </div>
            ...
            <div class="card">
                ...
                ${renderListaArquivos(state.drive.arquivos)}
            </div>
...
// ---------------------------------------------------------
function renderListaArquivos(lista) {

    if (lista.length === 0) return "<p>Nenhum arquivo.</p>";

    return lista.map(a => `
        <div class="card">
            <b>${a.nome}</b> ${botaoFavorito("arquivos", a.id)}
            <br>
            <small>Categoria: ${a.categoria}</small><br>
            <small>Tipo: ${a.tipo}</small><br>
            <small>Tamanho: ${(a.tamanho/1024).toFixed(1)} KB</small><br>
            <small>Enviado por: ${a.criador}</small><br>
            <button class="btn btn-primary" onclick="baixarArquivo(${a.id})">Baixar</button>
        </div>
    `).join("");
}
...
// PÁGINA DE CATEGORIA
// ---------------------------------------------------------
function paginaCategoriaDrive(cat) {

    const lista = state.drive.arquivos.filter(a => a.categoria === cat);

    return `
        <div class="container">
            ${UI.logo()}
            ${UI.titulo("Categoria: " + cat)}

            <div class="card">
                ${renderListaArquivos(lista)}
            </div>

            <button class="btn btn-secondary" onclick="mudarPagina('drive')">Voltar</button>
        </div>
    `;
}
...
// PÁGINA DE UPLOAD
// ---------------------------------------------------------
function paginaUploadArquivo() {

    return `
        <div class="container">
            ${UI.logo()}
            ${UI.titulo("Enviar Arquivo")}

            <div class="card">

                <label>Arquivo:</label>
                <input id="inputArquivoDrive" type="file">

                <label>Categoria:</label>
                <input id="inputCategoriaDrive" placeholder="Ex: Documentos, Relatórios, Fotos">

                <button class="btn btn-primary" onclick="salvarArquivoDrive()">Enviar</button>
            </div>
...
// ---------------------------------------------------------
function salvarArquivoDrive() {

    const input = document.getElementById("inputArquivoDrive");
    const categoria = document.getElementById("inputCategoriaDrive").value.trim();
    ...
    if (!categoria) {
        notificar("Informe uma categoria.", "error");
        return;
    }
    ...
    leitor.onload = function (e) {
        ...
        state.drive.arquivos.push({
            ...
            categoria,
            criador: state.usuarioLogado.usuario,
            dataUpload: new Date().toLocaleString()
        });

        salvarBackup();

        notificar("Arquivo enviado!", "success");
        auditar("Enviou arquivo", arquivo.name, "acao");
...
function baixarArquivo(id) {
    ...
    if (!a) {
        notificar("Arquivo não encontrado.", "error");
        return;
    }

    const link = document.createElement("a");
    link.href = a.conteudoBase64;
    link.download = a.nome;
    link.click();

    auditar("Baixou arquivo", a.nome, "acao");
}
...
function inserirBotaoDrive() {
    ...
    if (!menu.innerHTML.includes("Drive")) {
        menu.innerHTML = `
            <button class="btn btn-secondary" onclick="mudarPagina('drive')">
                📂 Drive
            </button>
        ` + menu.innerHTML;
    }
...
// INTEGRAR AO RENDER
// ---------------------------------------------------------
const renderOriginalParte40 = render;
render = function (param1, param2) {
    ...
    if (state.paginaAtual === "categoriaDrive") {
        renderSeguro(paginaCategoriaDrive(param1));
        inserirNavegacaoInteligente();
        return;
    }

    if (state.paginaAtual === "uploadArquivo") {
        renderSeguro(paginaUploadArquivo());
        inserirNavegacaoInteligente();
        return;
    }

    renderOriginalParte40(param1, param2);

    inserirBotaoDrive();
};
/*  
===========================================================
SUPER APP.JS — PARTE 41/20
PERFIS AVANÇADOS (FOTO, BIO, CONTATOS, VISUAL)
===========================================================
*/


// ---------------------------------------------------------
// GARANTIR CAMPOS AVANÇADOS EM USUÁRIOS
// ---------------------------------------------------------
...
// ---------------------------------------------------------
// PEGAR PERFIL AVANÇADO DE UM USUÁRIO
// ---------------------------------------------------------
function getPerfilAvancado(usuario) {
    const u = state.usuarios.find(x => x.usuario === usuario);
    if (!u) return null;
    garantirPerfisAvancados();
    return u.perfilAvancado;
}
...
// ---------------------------------------------------------
function avatarUsuario(usuario, tamanho = 40) {

    const p = getPerfilAvancado(usuario);
    if (!p || !p.foto) {
...
function paginaMeuPerfil() {
    ...
    const p = getPerfilAvancado(u.usuario);

    // permissões visuais
    const perms = u.permissoes || {};
    const listaPerms = Object.keys(perms).map(k => `
        <li>${k}: ${perms[k] ? "✅" : "❌"}</li>
    `).join("");

    // tarefas do usuário
    const minhasTarefas = (state.tarefas || []).filter(t => t.responsaveis?.includes(u.usuario));
    const htmlTarefas = minhasTarefas.slice(0,5).map(t => `
        <li>${t.titulo} (${t.status})</li>
    `).join("") || "<li>Nenhuma tarefa.</li>";

    // eventos do usuário (participante ou criador)
    const meusEventos = (state.calendario?.eventos || []).filter(e =>
        e.criador === u.usuario ||
        (e.presencas || []).some(p => p.usuario === u.usuario)
    );
    const htmlEventos = meusEventos.slice(0,5).map(e => `
        <li>${e.titulo} — ${e.data} ${e.hora || ""}</li>
    `).join("") || "<li>Nenhum evento.</li>";
    ...
    return `
        <div class="container">
            ...
            <div class="card">
                <h3>Permissões</h3>
                <ul>${listaPerms || "<li>Nenhuma permissão registrada.</li>"}</ul>
            </div>
        ...
        <div class="container">
            ...
            <div class="card" style="display:flex; align-items:center;">
                ...
                <div>
                    ...
                    <input id="inputFotoPerfil" type="file" accept="image/*">
                    <button class="btn btn-secondary" onclick="removerFotoPerfil()">Remover foto</button>
                </div>
            </div>

            <div class="card">
                <label>Cargo / Função:</label>
                <input id="pCargo" value="${p.cargo || ""}">

                <label>Bio / Descrição:</label>
                <textarea id="pBio">${p.bio || ""}</textarea>

                <label>Telefone:</label>
                <input id="pTelefone" value="${p.telefone || ""}">

                <label>E-mail:</label>
                <input id="pEmail" value="${p.email || ""}">

                <label>Observações:</label>
                <textarea id="pObs">${p.observacoes || ""}</textarea>

                <button class="btn btn-primary" onclick="salvarPerfilAvancado()">Salvar</button>
            </div>
...
// ---------------------------------------------------------
// SALVAR PERFIL AVANÇADO (TEXTO)
// ---------------------------------------------------------
function salvarPerfilAvancado() {
    ...
    p.cargo = document.getElementById("pCargo").value.trim();
    p.bio = document.getElementById("pBio").value.trim();
    p.telefone = document.getElementById("pTelefone").value.trim();
    p.email = document.getElementById("pEmail").value.trim();
    p.observacoes = document.getElementById("pObs").value.trim();

    salvarBackup();

    notificar("Perfil atualizado!", "success");
    auditar("Atualizou perfil", u.usuario, "acao");
...
function removerFotoPerfil() {
    ...
    mudarPagina("editarPerfil");
}


// ---------------------------------------------------------
// UPLOAD DE FOTO (CHAMADO AO SALVAR PERFIL SE TIVER ARQUIVO)
// ---------------------------------------------------------
function processarFotoPerfil(callback) {
...
// ---------------------------------------------------------
// AJUSTAR SALVAR PERFIL PARA CONSIDERAR FOTO
// ---------------------------------------------------------
const salvarPerfilAvancadoOriginal41 = salvarPerfilAvancado;
salvarPerfilAvancado = function () {
    processarFotoPerfil(() => {
        salvarPerfilAvancadoOriginal41();
    });
};


// ---------------------------------------------------------
// INTEGRAR AVATAR NO CHAT (SUBSTITUIR CABEÇALHO DAS MENSAGENS)
// ---------------------------------------------------------
const paginaChatConversaOriginal41 = paginaChatConversa;
paginaChatConversa = function (outroUsuario) {
    ...
    let htmlMsgs = msgs
        .map(m => `
            <div style="
                display:flex;
                ${m.de === u.usuario ? 'flex-direction:row-reverse;' : ''};
                margin:8px 0;
            ">
                ...
                ">
                    ...
                    ${m.texto}<br>
                    <small>${m.data}</small>
                    ${renderizarAnexos ? renderizarAnexos(m) : ""}
                </div>
            </div>
        `)
        .join("");
    ...
    return `
        <div class="container">
            ...
            <div class="card">
                ...
                ${typeof inputAnexoChat === "function" ? `
                    <hr>
                    <h4>Anexar arquivo</h4>
                    ${inputAnexoChat()}
                    <button class="btn btn-secondary" onclick="enviarAnexoChat('${outroUsuario}')">Enviar Anexo</button>
...
// ---------------------------------------------------------
// BOTÃO "MEU PERFIL" NO MENU LATERAL
// ---------------------------------------------------------
function inserirBotaoMeuPerfil() {
    ...
    if (!menu.innerHTML.includes("Meu Perfil")) {
        menu.innerHTML = `
            <button class="btn btn-secondary" onclick="mudarPagina('meuPerfil')">
                👤 Meu Perfil
            </button>
        ` + menu.innerHTML;
    }
...
// INTEGRAR AO RENDER
// ---------------------------------------------------------
const renderOriginalParte41 = render;
render = function (param1, param2) {
    ...
    renderOriginalParte41(param1, param2);

    inserirBotaoMeuPerfil();
};
/*  
===========================================================
SUPER APP.JS — PARTE 41.1
DIRETÓRIO DE USUÁRIOS (LISTA COM AVATAR + BUSCA + CHAT)
===========================================================
*/
...
// ---------------------------------------------------------
// FILTRAR DIRETÓRIO EM TEMPO REAL
// ---------------------------------------------------------
...
// ---------------------------------------------------------
// PÁGINA: PERFIL DE OUTRO USUÁRIO
// ---------------------------------------------------------
...
// BOTÃO NO MENU LATERAL
// ---------------------------------------------------------
function inserirBotaoDiretorioUsuarios() {
    ...
    if (!menu.innerHTML.includes("Diretório")) {
        menu.innerHTML = `
            <button class="btn btn-secondary" onclick="mudarPagina('diretorioUsuarios')">
                👥 Diretório
            </button>
        ` + menu.innerHTML;
    }
...
// INTEGRAR AO RENDER
// ---------------------------------------------------------
const renderOriginalParte411 = render;
render = function (param1, param2) {
    ...
    renderOriginalParte411(param1, param2);

    inserirBotaoDiretorioUsuarios();
};